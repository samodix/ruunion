...
<?php while ( have_posts() ) : the_post();
	...
	$image    = ruunion_theme_film_image( $film_id, 'full' ) ?: ruunion_theme_asset_image( 'stock/cinematic-reference.jpg' );
...
<section class="ru-film-hero ru-film-hero--premium">
	<div class="ru-container ru-film-hero__grid">
		<div data-ru-reveal>
			<span class="ru-pill"><?php echo esc_html( $status ); ?> · <?php echo esc_html( $year ); ?></span>
		...
		<div class="ru-film-hero__poster has-image" style="background-image:url('<?php echo esc_url( $image ); ?>')" data-ru-parallax><span>Une histoire à faire vivre</span></div>
	</div>
</section>
<section class="ru-section">
	<div class="ru-container ru-film-layout">
		<div>
			...
			<span class="ru-eyebrow">Photos · Galerie · Coulisses</span>
			<h2><?php esc_html_e( 'Galerie du projet', 'ruunion-theme' ); ?></h2>
			<div class="ru-gallery ru-gallery--cinematic">
				<?php if ( $gallery ) : foreach ( array_slice( $gallery, 0, 8 ) as $index => $gallery_url ) : ?><figure data-ru-reveal><img src="<?php echo esc_url( $gallery_url ); ?>" alt="<?php echo esc_attr( sprintf( 'Photo de tournage %1$s — image %2$d', get_the_title(), $index + 1 ) ); ?>" loading="lazy"></figure><?php endforeach; else : foreach ( array( 'stock/cinematic-reference.jpg', 'team/ROLAND USTOK.webp', 'team/MUSTAFA OZGUN - Réalisateur.webp' ) as $index => $fallback ) : ?><figure data-ru-reveal><img src="<?php echo esc_url( ruunion_theme_asset_image( $fallback ) ); ?>" alt="<?php echo esc_attr( sprintf( 'Image de coulisses %d', $index + 1 ) ); ?>" loading="lazy"></figure><?php endforeach; endif; ?>
			</div>
			<div id="making-of" class="ru-impact ru-making-of" data-ru-reveal><span class="ru-eyebrow">Making Of</span><h2>Pourquoi soutenir ce film ?</h2><p>Ce projet se construit avec ses futurs publics. Les soutiens financent l’écriture, le tournage, la postproduction, la création de supports et les projections accompagnées sur le terrain.</p></div>
